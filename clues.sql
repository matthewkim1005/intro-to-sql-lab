...
-- recognized language. Check our databases and find out what language is spoken in this country, so we can call in a translator to work with you.

-- Write SQL query here

SELECT countries.name AS countries, countrylanguages.language
FROM countries
JOIN countrylanguages ON countries.code = countrylanguages.countrycode 
WHERE countries.population > 0 AND countries.region = 'Southern Europe' ORDER BY population ASC
LIMIT 1;
...
WHERE countries.region = 'Southern Europe' AND countrylanguages.language = 'Italian' LIMIT 1;
...
WHERE countrycode = 'SMR' AND name <> 'San Marino' LIMIT 1;
...
WHERE countries.continent = 'South America' AND cities.name LIKE 'Serra%' LIMIT 1;
